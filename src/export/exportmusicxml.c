...
gint exportmusicXML(gchar *thefilename, DenemoProject *gui)
{
	...
	for (i = 0, curStaff = si->thescore; curStaff != NULL; i++, curStaff = curStaff->next)
	{
		...
		GList *lyrics = NULL; // le lyrics (intere, non in sillabe) di un solo sistema
		// GList *all_lyrics = NULL;
		GList *list_syllabe = NULL; // una lista con tutte le sillabe
		GList *list_sillabe_type = NULL;
		GList *l = NULL;
		...
		if (has_lyrics)
		{
			...
			{
				gchar *text = get_text_from_view(l->data);
				if (text)
				{
					split_and_process_lyrics(&list_syllabe, &list_sillabe_type, text);

					// test
					GList *pointer_type = list_sillabe_type;
					for (GList *ll = list_syllabe; ll; ll = ll->next)
					{
						printf("-> %s - %s\n", ll->data, pointer_type->data);
						pointer_type = pointer_type->next;
					}
				}
			}
		}

		GList *syllabe_iterator = list_syllabe; // Inizializza l'iteratore per list_syllabe
		GList *type_iterator = list_sillabe_type;
		// Fine mio codice

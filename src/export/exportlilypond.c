...
{
  int i;
  g_string_append_printf(figures, "%d", duration);
  for (i = 0; i < numdots; i++)
...
// NAVANC
#define DIRECTIVES_INSERT_EDITABLE_AFFIX(field)                                                                                                                     \
  static void                                                                                                                                                       \
  ...
  {                                                                                                                                                                 \
    ...
    {                                                                                                                                                               \
      ...
      {                                                                                                                                                             \
        if (popen_braces)                                                                                                                                           \
          *popen_braces += brace_count(directive->field->str);                                                                                                      \
        insert_editable(&directive->field, directive->field->str, iter, gui, lily_for_obj, type, movement_count, measurenum, voice_count, objnum, num, midcoffset); \
      }                                                                                                                                                             \
    }                                                                                                                                                               \
  }
...
static void directives_insert_prefix_altaffix_editable(GList *directives, gint *popen_braces, gint *pprevduration, GtkTextIter *iter, gboolean override, GString *lily_for_obj,
                                                       DenemoTargetType type, gint movement_count, gint measurenum, gint voice_count, gint objnum, gint midcoffset, guint sbid)
{
  DenemoProject *gui = Denemo.project;
  GList *g = directives;
  ...
  for (num = 0; g; g = g->next, num++)
  {
    ...
    if (directive->prefix && directive->prefix->len)
    {
      ...
      insert_editable(&directive->prefix, directive->prefix->str, iter, gui, lily_for_obj, type, movement_count, measurenum, voice_count, objnum, num, midcoffset);
    }
  }
}
 ...
 */
static gint
generate_lily_for_obj(DenemoProject *gui, GtkTextIter *iter, DenemoObject *curobj, gint *pprevduration, 
gint *pprevnumdots, gchar **pclefname, gchar **pkeyname, gint *pcur_stime1, gint *pcur_stime2, 
gint *pgrace_status, GString *figures, GString *fakechords, GtkTextMark *curmark, gpointer curobjnode, 
gint movement_count, gint measurenum, gint voice_count, gint objnum, guint sbid, gboolean firstnote)
{
  GString *lily_for_obj = g_string_new("");
  GString *ret = g_string_new(""); // no longer returned, instead put into *music
#define outputret                                                                                    \
  gtk_text_buffer_insert_with_tags_by_name(Denemo.textbuffer, iter, ret->str, -1, INEDITABLE, NULL), \
      g_string_append(lily_for_obj, ret->str);                                                       \
  open_braces += brace_count(ret->str),                                                              \
      g_string_assign(ret, "")
#define output(astring)                                                                               \
  (gtk_text_buffer_insert_with_tags_by_name(Denemo.textbuffer, iter, astring, -1, INEDITABLE, NULL)); \
...
#define NAVANC(type, offset)                                                                                             \
  place_navigation_anchor(curmark, (gpointer)curobjnode, movement_count, measurenum, voice_count, objnum, type, offset); \
  gtk_text_buffer_get_iter_at_mark(Denemo.textbuffer, iter, curmark);
  ...
  case CHORD:
  {
    ...
    /* prefix is before duration unless AFFIX override is set so this takes the affix ones unless overriding lilypond*/
      ...
      else /* there are notes */
      {
        ...
        for (notenode = pchord->notes; notenode; notenode = notenode->next)
        {
          ...
          { // not skip all LilyPond output for this note
            ...
            last_mid_c_offset = mid_c_offset;
            ...
            // mio codice il firstnote e l'else
            if (firstnote)
            {
              octave = mid_c_offsettooctave(mid_c_offset);
              printf("mid_c_offset -> %d - octave -> %d\n", mid_c_offset, octave);
              if (octave < 0)
                for (; octave; octave++)
                  g_string_append_printf(ret, ",");
              else
            ...
            {
              // Calcola la differenza tra la nota attuale e quella precedente
              gint rel_diff = mid_c_offset - last_mid_c_offset;

              // Inizializza il contatore dei token (ovvero il numero di apici o virgole da aggiungere)
              gint token_number = 0;

              // Se la differenza tra le note è maggiore di 3 o minore di -3, aumentiamo il contatore
              if (rel_diff > 3) {
                token_number = (rel_diff - 3) / 7 + 1;

              }
              else if (rel_diff < -3) {
                token_number = - 1 + (rel_diff + 3) / 7;

              }
              
              printf("note: %d, note_diff: %d, token_number: %d\n", mid_c_offset, rel_diff, token_number);
              // Se token_number > 0, aggiungiamo i giusti apici ('), altrimenti aggiungiamo virgole (,)
              if (token_number > 0)
              {
                for (gint i = 0; i < token_number; i++)
                {
                  g_string_append_printf(ret, "'");
                  printf("'");
                }
...
printf("\n");
              // Aggiorna i valori di mid_c_offset e octave per la prossima iterazione
              last_mid_c_offset = mid_c_offset;
            }
            NAVANC(TARGET_NOTE, mid_c_offset); // we target the note
            outputret;
        ...
        directives_insert_prefix_altaffix_editable(pchord->directives, &open_braces, &prevduration, iter, TRUE, lily_for_obj, TARGET_CHORD, movement_count, measurenum, voice_count, objnum, 0, sbid);
        directives_insert_postfix_editable(pchord->directives, &open_braces, &prevduration, iter, FALSE, lily_for_obj, TARGET_CHORD, movement_count, measurenum, voice_count, objnum, 0, sbid);
        directives_insert_postfix_altaffix_editable(pchord->directives, &open_braces, &prevduration, iter, FALSE, lily_for_obj, TARGET_CHORD, movement_count, measurenum, voice_count, objnum, 0, sbid);
  ...
  case STEMDIRECTIVE:
  {
    ...
    if (override)
      ...
      case DENEMO_STEMDOWN:
        g_string_append_printf(ret, "%s\\stemDown"
                                    "%s",
                               prestem_string, poststem_string);
        break;
      case DENEMO_STEMBOTH:
        g_string_append_printf(ret, "%s\\stemNeutral"
                                    "%s",
                               prestem_string, poststem_string);
        break;
      case DENEMO_STEMUP:
        g_string_append_printf(ret, "%s\\stemUp"
                                    "%s",
                               prestem_string, poststem_string);
        break;
      }
  ...
  break;
  case LILYDIRECTIVE:; // handled in the if block
    break;
  default:
    break;
  }
...
outputStaff(DenemoProject *gui, DenemoStaff *curstaffstruct, gint start, gint end, gchar *movement, gchar *voice, gint movement_count, gint voice_count, DenemoScoreblock *sb)
{
  ...
  { /* standard staff-prolog */
    ...
    // mio codice il relative
    g_string_append_printf(staff_str, "%s%s = \\relative {\n", movement, voice);
...
#define OUTPUT_LILY(what)                                                                                                               \
  if (directive->what && directive->what->len && !wrong_layout(directive, sb->id) && (!(directive->override & DENEMO_OVERRIDE_HIDDEN))) \
  {                                                                                                                                     \
    gtk_text_buffer_get_iter_at_mark(Denemo.textbuffer, &iter, curmark);                                                                \
    GtkTextChildAnchor *objanc = gtk_text_buffer_create_child_anchor(Denemo.textbuffer, &iter);                                         \
...
#undef OUTPUT_LILY
          ...
          else
          {
            ...
            open_braces += generate_lily_for_obj(gui, &iter, curobj, &prevduration, &prevnumdots, &clefname,
                                                 &keyname, &cur_stime1, &cur_stime2, &grace_status, figures, fakechords, curmark,
                                                 (gpointer)curobjnode, ABS(movement_count), measurenum, ABS(voice_count), ABS(objnum), sb->id, firstnote);
         firstnote = FALSE;
        ...
        { // at end of measure
          ...
          if ((curobjnode != NULL) && ((curobj == NULL) || curobj->type == CHORD)) /* if it doesn't end with a note or rest, the user may want to choose their own barline style, let them */
          {
            ...
            else if (curstaffstruct->voicecontrol == DENEMO_PRIMARY)
...
void merge_lily_strings(DenemoProject *gui)
{
  // g_debug("Merge...\n");
  GList *g;
...
void set_voice_definition(GString *str, DenemoStaff *curstaffstruct, gchar *voicetag)
{

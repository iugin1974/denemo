...
scheme_toggle_midi_in_listening (void)
{
  if (Denemo.keyboard_state == GDK_SHIFT_MASK)
	...
	midi_in_adjust (GDK_SHIFT_MASK);
  return scm_from_bool (Denemo.keyboard_state == GDK_SHIFT_MASK);
}
...
scheme_midi_in_checking (void)
{
  ...
  return scm_from_bool (Denemo.keyboard_state == GDK_CONTROL_MASK);
}
...
SCM scheme_synchronize_recording (void)
{
  DenemoProject *gui = Denemo.project;
  ...
  if (si->recording && (si->recording->type == DENEMO_RECORDING_MIDI) && si->recording->notes)
    {
      ...
      gdouble seconds = thenote->timing / ((gdouble) si->recording->samplerate);
      thenote = (DenemoRecordedNote *)si->recording->notes->data;
      seconds -= thenote->timing / ((gdouble) si->recording->samplerate);
      scm = scm_from_double (seconds);
    }
  return scm;
}

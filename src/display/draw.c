...
//    repeat = TRUE;              /* the first system is already drawn, so it is too late to discover that we needed to scale it */
  *itp->right = itp->measurenum - 1;
  ...
  if ((!thestaff->fixed_height) && (space_above>-1) && (itp->staffnum != si->top_staff))
			thestaff->space_above = MAX (space_above, thestaff->space_above);
  if ((!thestaff->fixed_height) && (space_below>-1))
...
draw_playback_marker (cairo_t * cr, gint color, gdouble alpha, gint pos, gint yy, gint line_height)
{
  ...
  switch (color)
    {
    case GREEN:
      drawlargetext_cr (cr, _("Ctrl+Alt-Drag Start"), pos - 100, yy + 7*LINE_SPACE);
      break;
    case RED:

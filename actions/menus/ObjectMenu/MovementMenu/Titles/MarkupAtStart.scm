;;;MarkupAtStart
(let* ((tag "MarkupAtStart")(themarkup #f)(data (d-DirectiveGet-movementcontrol-data tag)))
(disp "eneter with " data "\n\n")
        (d-DirectivePut-movementcontrol-override tag (logior
            DENEMO_OVERRIDE_GRAPHIC DENEMO_OVERRIDE_TAGEDIT))
        (d-DirectivePut-movementcontrol-display tag (_ "Markup At Start"))
        (if data
        ...
        (if themarkup
            (begin
                ...
                (d-DirectivePut-movementcontrol-prefix tag (string-append "\\markup \\column { " themarkup " }"))
                (d-SetSaved #f))
